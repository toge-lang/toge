...
function tokenize(source) { 
  ...
  while(pos < source.length) {
    ...
    if(char === '(') {tokens.push({type: "LPAREN", value: '('}); pos++} // left paranthesis token
      ...
      if(pos + 1 < source.length && source[pos+1] === '=') {tokens.push({type: "PLUS_EQ", value: '+='}); pos += 2} // plus equals token
      else if(pos + 1 < source.length && source[pos+1] === char && pos + 2 < source.length && source[pos+2] === char) {tokens.push({type: "AND_GATE", value: '+++'}); pos += 3} // and gate token
